...
function NovoAtivo() {
  ...
  const filteredCategories = DEPRECIATION_CATEGORIES.filter(
    (c) => c.nature === nature,
  );
      ...
      <div className="p-6 lg:p-10 max-w-5xl space-y-6">
        <header>
          ...
          <h1 className="text-3xl font-display font-semibold mt-1">
            Registar Ativo Imobilizado
          </h1>
          <p className="text-muted-foreground mt-1">
            Conforme o Plano Geral de Contabilidade Angolano (Classe 1 — Imobilizações).
          </p>
        </header>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Deixe vazio para usar {autoCode}.
                  </p>
                </div>
                <div className="space-y-1.5">
                  <Label>Natureza</Label>
                  <Select value={nature} onValueChange={(v) => {
            ...
            <section className="rounded-xl border border-border bg-card p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Início da amortização (Art.º 3.º n.º 2).
                  </p>
                </div>
                <div className="space-y-1.5">
            ...
            <div className="rounded-xl border border-border bg-card p-6 space-y-3 sticky top-6">
              ...
              {cat && preview ? (
                <dl className="text-sm space-y-2">
                  <Row label="Categoria" value={`${cat.section}`} />
...
function Row({ label, value, strong, mono }: { label: string; value: string; strong?: boolean; mono?: boolean }) {
  return (
    <div className="flex justify-between gap-3">
      ...
      <dd className={`text-right ${strong ? "font-semibold" : ""} ${mono ? "font-mono text-xs" : "tabular"}`}>
        {value}
      </dd>
    </div>
  );
